...
(async () => {
    // Get the URL file path and output directory from the command line arguments
    const args = process.argv.slice(2); // Slice to ignore the first two elements (node and script paths)
    if (args.length < 2) {
        console.error('Please provide the path to the URL file and the output directory.');
        process.exit(1);
    }

    const urlFilePath = args[0]; // The first argument is the path to the URL file
    const outputDir = args[1];    // The second argument is the output directory
    ...
    try {
        // Read the URLs from the provided file path
        const urls = fs.readFileSync(urlFilePath, 'utf-8')
            .split('\n')
            .map(line => line.trim())
            .filter(line => line) // Filters out empty lines
            .map(url => url.startsWith('http://') || url.startsWith('https://') ? url : `https://${url}`); // Ensure each URL starts with http:// or https://
        ...
        for (const targetUrl of urls) {
            ...
            while (attempt < RETRY_LIMIT && !success) {
                ...
                try {
                    ...
                    fs.writeFileSync(`${outputDir}/${baseFilename}.json`, JSON.stringify(successfulClicks, null, 4));

                    // Take a screenshot of the page
                    await page.screenshot({ path: `${outputDir}/${baseFilename}.png` });
